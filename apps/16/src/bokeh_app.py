"""
VOID App 16 - Flussi Migratori San Michele al Tagliamento
...
def get_national(df):
    ...
    # Rename
    df['COMUNE PROVENIENZA'] = df['DESCRIZIONE COMUNE PROVENIENZA']

    # Filtra solo Stato == ITALIA
    df_c = df[df['STATO ESTERO DI PROVENIENZA'].isnull()]
    ...
    g_c['COMUNE DESTINAZIONE'] = 'SAN MICHELE AL TAGLIAMENTO'

    # Filter
    g_c = g_c[g_c['Flusso']>=100]
...
def get_international(df):
    ...
    # Rename
    df['STATO PROVENIENZA'] = df['STATO ESTERO DI PROVENIENZA']

    # Stato != ITALIA
    df_s = df[~df['STATO ESTERO DI PROVENIENZA'].isnull()]

    # Raggruppa per Stato di Provenienza
    g_s = df_s.groupby('STATO PROVENIENZA').size().reset_index(name='Flusso')

    # Aggiungi Colonna Comune di Destinazione
    g_s['COMUNE DESTINAZIONE'] = 'SAN MICHELE AL TAGLIAMENTO'
    # Filtra
