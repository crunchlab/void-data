"""
VOID App 17 - Flussi Migratori San Stino di Livenza
...
def get_national(df):
    ...
    # Rename
    df['COMUNE PROVENIENZA'] = df['COMUNE DI PROVENIENZA']

    # Filtra solo Stato == ITALIA
    df_c = df[df['STATO DI PROVENIENZA'] == 'ITALIA']
    ...
    g_c['COMUNE DESTINAZIONE'] = 'SAN STINO DI LIVENZA'

    # Filter
    g_c = g_c[g_c['Flusso']>=50]
...
def get_international(df):
    ...
    # Rename
    df['STATO PROVENIENZA'] = df['STATO DI PROVENIENZA']
    ...
    # Aggiungi Colonna Comune di Destinazione
    g_s['COMUNE DESTINAZIONE'] = 'SAN STINO DI LIVENZA'
    # Filtra
    g_s = g_s[g_s['Flusso']>=20]
